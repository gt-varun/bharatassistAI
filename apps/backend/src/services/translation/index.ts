 ...
 * administrative wording; Gemini is the easiest to obtain a free key for; the
 * local IndicTrans2 server is last because it requires a machine to be running
 * it, but it is the only one that never expires.
 */
const FALLBACK_ORDER = ['bhashini', 'gemini', 'indictrans'];
