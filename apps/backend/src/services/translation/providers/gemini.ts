...
// A real Gemini model name, deliberately not imported from geminiClient.ts's
// `CHAT_MODEL` — that constant now names whichever Groq model chat
// completions use (see geminiClient.ts's module comment on the provider
// switch), and passing a Groq model name to the Gemini SDK would fail.
